...
impl Maze {
    ...
    fn fix(&mut self, pos: &Pos) {
        use Direction::*;
        use Tile::*;
        let connections = vec![N, E, S, W]
            .iter()
            .map(|direction| {
...
fn part1(mut maze: Maze) -> u32 {
    let start = maze.start().expect("no animal");
    maze.fix(&start);
    let (rows, cols) = maze.dim().as_index();
